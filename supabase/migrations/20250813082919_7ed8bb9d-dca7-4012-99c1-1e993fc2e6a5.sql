...
-- This allows admins to see who is in their org without exposing sensitive details
...
-- Create a restricted admin view policy that only allows viewing basic info
-- This replaces the overly permissive "view all profiles" policy
CREATE POLICY "Org admins can view basic member info"
ON public.profiles
FOR SELECT
USING (
  -- Admins can only see basic info (not email) of their org members
  is_org_admin(org_id) AND 
  -- But only if they're accessing through the proper function
  -- This prevents direct table access to sensitive fields
  current_setting('app.admin_access', true) = 'basic_only'
);

-- Create a policy for admins to update only specific fields (not email)
CREATE POLICY "Org admins can update member roles and names"
ON public.profiles
FOR UPDATE
USING (is_org_admin(org_id))
WITH CHECK (
  is_org_admin(org_id) AND
  -- Prevent admins from changing emails - only user can do that
