...
-- Add default modules for the organization (using text values instead of enum)
INSERT INTO public.organization_modules (org_id, module)
SELECT o.id, 'fundraising'
...
SELECT o.id, 'General', 'Default department for organization members', 
       (SELECT p.id FROM public.profiles p WHERE p.org_id = o.id ORDER BY p.created_at ASC LIMIT 1)
FROM public.organizations o
WHERE NOT EXISTS (
