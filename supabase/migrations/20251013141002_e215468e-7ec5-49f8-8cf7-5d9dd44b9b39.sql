...
-- Insert sample vendors for procurement dashboard
...
BEGIN
  -- Get vendor IDs
  SELECT id INTO v_vendor1 FROM vendors WHERE vendor_name = 'Office Supply Co.' AND org_id = v_org LIMIT 1;
  SELECT id INTO v_vendor2 FROM vendors WHERE vendor_name = 'Tech Solutions Inc.' AND org_id = v_org LIMIT 1;
  SELECT id INTO v_vendor3 FROM vendors WHERE vendor_name = 'Marketing Print Ltd.' AND org_id = v_org LIMIT 1;
  ...
  -- Get requisition IDs
  SELECT id INTO v_req1 FROM requisitions WHERE requisition_number = 'REQ-2025-001' AND org_id = v_org LIMIT 1;
  SELECT id INTO v_req2 FROM requisitions WHERE requisition_number = 'REQ-2025-002' AND org_id = v_org LIMIT 1;
  SELECT id INTO v_req3 FROM requisitions WHERE requisition_number = 'REQ-2025-003' AND org_id = v_org LIMIT 1;
  ...
  -- Get PO IDs
  SELECT id INTO v_po1 FROM purchase_orders WHERE po_number = 'PO-2025-001' AND org_id = v_org LIMIT 1;
  SELECT id INTO v_po2 FROM purchase_orders WHERE po_number = 'PO-2025-002' AND org_id = v_org LIMIT 1;
  SELECT id INTO v_po3 FROM purchase_orders WHERE po_number = 'PO-2025-003' AND org_id = v_org LIMIT 1;

  -- Insert invoices (without remaining_amount as it's generated)
  ...
  -- Insert deliveries
  INSERT INTO deliveries (org_id, delivery_number, po_id, status, scheduled_date, tracking_number, carrier)
  VALUES 
    (v_org, 'DEL-2025-001', v_po1, 'scheduled', CURRENT_DATE + 10, 'TRK123456789', 'FedEx'),
    (v_org, 'DEL-2025-002', v_po2, 'scheduled', CURRENT_DATE + 5, 'TRK987654321', 'UPS'),
    (v_org, 'DEL-2025-003', v_po3, 'in_transit', CURRENT_DATE + 2, 'TRK456789123', 'DHL')
  ON CONFLICT DO NOTHING;
