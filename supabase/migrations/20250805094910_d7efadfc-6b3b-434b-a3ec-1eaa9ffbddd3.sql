-- Create organizations table (skip enum creation if it already exists)
...
INSERT INTO public.pricing_tiers (name, display_name, description, features) 
SELECT 'small_teams', 'Small Teams', 'Perfect for small organizations getting started', '["Up to 5 users", "Basic modules", "Email support", "5GB storage"]'
WHERE NOT EXISTS (SELECT 1 FROM public.pricing_tiers WHERE name = 'small_teams');

INSERT INTO public.pricing_tiers (name, display_name, description, features) 
SELECT 'growing_teams', 'Growing Teams', 'Great for expanding organizations', '["Up to 25 users", "Advanced modules", "Priority support", "50GB storage", "Custom reports"]'
WHERE NOT EXISTS (SELECT 1 FROM public.pricing_tiers WHERE name = 'growing_teams');

INSERT INTO public.pricing_tiers (name, display_name, description, features) 
SELECT 'enterprise_branding', 'Enterprise & Branding', 'Full-featured for large organizations', '["Unlimited users", "All modules", "24/7 support", "Unlimited storage", "Custom branding", "API access"]'
WHERE NOT EXISTS (SELECT 1 FROM public.pricing_tiers WHERE name = 'enterprise_branding');
...
-- Create or replace all policies (drop existing ones first)
...
DROP POLICY IF EXISTS "Users can view their organization pricing" ON public.organization_pricing;
DROP POLICY IF EXISTS "Users can manage their organization pricing" ON public.organization_pricing;
DROP POLICY IF EXISTS "Users can view their own registration progress" ON public.registration_progress;
DROP POLICY IF EXISTS "Users can manage their own registration progress" ON public.registration_progress;
...
DROP TRIGGER IF EXISTS update_registration_progress_updated_at ON public.registration_progress;
