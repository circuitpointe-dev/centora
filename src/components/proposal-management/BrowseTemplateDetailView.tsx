...
const BrowseTemplateDetailView: React.FC<BrowseTemplateDetailViewProps> = ({
    ...
    const handleUseTemplate = async () => {
        if (isUsing) return; // prevent duplicate submissions
        setIsUsing(true);
        // Professional flow: immediately create a draft and navigate by id
        const toField = (name: string, value: string) => ({ id: name.toLowerCase().replace(/\s+/g, '-'), name, value });
        const overview_fields = [
            toField('Summary', template.description || ''),
            toField('Objectives', ''),
        ];
        const narrative_fields = [toField('Template Notes', template.description || '')];

        try {
            const created = await createProposal.mutateAsync({
                name: template.title,
                title: template.title,
                opportunity_id: (creationContext as any)?.opportunityId,
                overview_fields,
                narrative_fields,
                budget_currency: 'USD',
            ...
            navigate("/dashboard/fundraising/manual-proposal-creation", { state: { prefilledData: templateData } });
        } finally {
    ...
    return (
        <div className="w-full max-w-7xl mx-auto p-4 space-y-6">
            <ProposalDetailHeader
                ...
                buttonText={isUsing ? "Creating..." : "Use Template"}
