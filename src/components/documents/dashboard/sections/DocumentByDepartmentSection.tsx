...
export const DocumentByDepartmentSection = (): JSX.Element => {
  ...
  // Set card and chart heights for visual alignment
  const cardHeight = 412; // px for notification card parity
  const chartHeight = 268; // px for bars (was 309px, reduced)
  const totalYAxisLabels = departmentData.length;
  const barVerticalGap = (chartHeight - totalYAxisLabels * 40) / (totalYAxisLabels - 1);
    ...
    <Card className="border border-gray-200 shadow-sm rounded-sm h-[412px] min-h-[412px] flex flex-col">
      ...
      <CardContent className="p-0 flex-1 flex flex-col overflow-hidden px-8 pb-8 pt-0">
        <div className="flex-grow flex flex-col justify-center relative">
          <div className="relative" style={{ height: `${chartHeight + 28}px` }}>
            {/* Chart container */}
            <div className="flex" style={{ minHeight: `${chartHeight}px`, position: "relative", marginLeft: "68px" }}>
              ...
              <div className="relative w-full" style={{ height: `${chartHeight}px` }}>
                ...
                {departmentData.map((dept, index) => {
                  ...
                  const top = index * (40 + barVerticalGap) + 2;

                  return (
                    <div
                      key={`bar-${index}`}
                      className="absolute h-10 left-2 bg-[#4d7dfd] rounded-sm"
                      style={{
                        width: `${width}px`,
                        top: `${top}px`,
                      }}
