...
const CloseGrantPage = () => {
  ...
  const handleCloseGrant = () => setCloseDialogOpen(true);
  ...
  const getStatusIcon = (status) => {
    if (status === 'Completed' || status === 'Submitted') {
      return <div className="h-4 w-4 bg-green-600 rounded-full flex items-center justify-center"><div className="h-2 w-2 bg-white rounded-full"></div></div>;
    }
  ...
  const renderPagination = (tableType, totalPages, currentPageNum) => {
  ...
  return (
    <div className="space-y-8 px-6 py-8">
      ...
      {/* Table Sections */}
      {/* Repeat for Reporting Schedule, Disbursement Schedule, Compliance Requirements using the same layout */}

      {/* Final Actions */}
      <div className="flex justify-end gap-4 pt-6 border-t border-gray-200">
        <Button variant="outline" onClick={handleCancel} className="border-gray-300 text-gray-700">Cancel</Button>
        <Button onClick={handleCloseGrant} className="bg-red-600 hover:bg-red-700">Close Grant</Button>
      </div>
